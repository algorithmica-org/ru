...
**Loop unrolling.** Добавление флага `unroll-loops` (`#pragma GCC optimize("unroll-loops")`) позволяет компилятору делать «[размотку](https://en.wikipedia.org/wiki/Loop_unrolling)» циклов, то есть преобразовывать код вида:
