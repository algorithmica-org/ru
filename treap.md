...
Декартово дерево удобно писать на указателях и структурах. Поэтому мы [рассказали](https://algorithmica.org/ru/segtree) дерево отрезков на указателях, а не стандартную [рекурсию на 5 параметрах](http://e-maxx.ru/algo/segment_tree). Не знаете, что это такое — посмотрите в предыдущем конспекте.
